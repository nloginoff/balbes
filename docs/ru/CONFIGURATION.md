...
# безопасное экранирование и лёгкая разметка (**жирный**, `код`, блоки ```); при ошибке разбора HTML
# (BadRequest) — повтор без parse_mode: plain-текст из отклонённого HTML (без литералов **/_ из сырого ответа модели).
# Лимит длины фрагмента — 4096 UTF-16 code units (эмодзи считаются за 2).
# Конвертация: **/__ жирный, ***жирный курсив***, */_ курсив,
# ~~зачёркнутое~~, ||спойлер||, [текст](https://…), цитата строкой "> …" или "MD:/HTML: > …",
# ``` блоки, вложенные <b>/<i>/…, <tg-spoiler>; голые https://… как ссылка; одна строка print(...) → `код`.
# В ``||спойлер||`` ссылку пишите так, чтобы URL не слипался с закрывающими ``||`` (конвертер срезает хвостовые ``|`` у href).
...
Секции `server_commands` / `server_commands_ask` в workspace **объединяются** с глобальным списком из `providers.yaml`: поле `allowed_commands` **дополняет** (union), а не заменяет весь вайтлист — можно указывать только лишние шаблоны, например `date`, без копирования полного перечня.

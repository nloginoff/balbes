...
Сервис [`services/blogger`](../../services/blogger): посты, бизнес-бот, очередь публикаций. **Публикация в каналы** (после одобрения) идёт **тем же** ботом, что и бизнес-бот (`BUSINESS_BOT_TOKEN` — бот админ в каналах; если не задан — fallback на `TELEGRAM_BOT_TOKEN`). Превью черновиков с кнопками одобрения у владельца уходят **в чат с бизнес-ботом** (при `BUSINESS_BOT_TOKEN`), а не в диалог с оркестратором. **Диалог в личке с бизнес-ботом** и **HTTP-делегирование** (`POST /api/v1/agent/execute`) используют тот же [`ToolDispatcher`](../../shared/agent_tools/registry.py), что и оркестратор: набор инструментов задаётся записью **`agents` → `id: blogger`** и **`tools_allowlist`** в [`config/providers.yaml`](../../config/providers.yaml) (без отдельного дублирующего списка tool-only в коде блогера). **Системный промпт** для этих двух режимов собирается в том же порядке bootstrap-файлов, что у [`AgentWorkspace`](../../services/orchestrator/workspace.py): шпаргалка в репо — [`services/blogger/bbot_bootstrap/`](../../services/blogger/bbot_bootstrap/); при наличии файлов в `data/agents/blogger/` они **перекрывают** дефолт поштучно. Не из зашитых строк в Python. **Публичные dev-посты** (чаты `balbes` / `coder` в Memory, дедуп заголовков, пакет черновиков) настраиваются блоком **`blogger.dev_blog`** в том же [`config/providers.yaml`](../../config/providers.yaml); сценарий **`/generate`** и `POST /api/v1/posts/generate` могут создать **несколько** черновиков за вызов; шаблон инструкций — `POST_DEV_AGENT.md` (рядом с `IDENTITY.md` / `SOUL.md`). История DM с владельцем в Memory Service хранится под **`user_id` = `blogger_<telegram_user_id>`** — см. [`shared/telegram_app/memory_namespace.py`](../../shared/telegram_app/memory_namespace.py). Старый префикс `bbot_<id>` при чтении ещё учитывается для миграции. Общий список slash-команд и порядок меню — [`shared/telegram_app/telegram_command_matrix.py`](../../shared/telegram_app/telegram_command_matrix.py); включение/выключение — блок `telegram:` в [`config/agents/blogger.yaml`](../../config/agents/blogger.yaml) (в т.ч. `debug_command` для `/debug`). Подробнее по настройкам: [`docs/ru/CONFIGURATION.md`](CONFIGURATION.md).

...
**Параллельные запросы:** для одного `user_id` одновременно выполняется **не больше одной** foreground-задачи (источник не `heartbeat`). Повторный `POST` до завершения первой сразу возвращает `status: "failed"` с подсказкой дождаться или отменить текущую задачу (`POST /api/v1/tasks/cancel?user_id=...`, в Telegram — **`/stop`**). Отмена снимается между раундами LLM и между вызовами инструментов; один «долгий» сетевой вызов внутри раунда ей не прерывается. Клиент **Telegram-бот** ждёт HTTP-ответ с таймаутом чтения (константа `ORCHESTRATOR_POST_TIMEOUT_SEC`, по умолчанию 120 с); при таймауте бот отправляет в оркестратор **cancel** для того же `user_id`, чтобы не копить «зомби»-сессии после обрыва со стороны клиента.

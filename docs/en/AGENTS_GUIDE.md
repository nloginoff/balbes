...
- **`render_chart`** — deterministic charts: pass a nested **`spec`** *or* the same fields at the **top level** (`kind`, `series`, `title`, …). `kind` line | scatter | bar | histogram. For function plots in **line**/**scatter** «school» coordinates: `style: "school"` and/or `axes_origin: true` (grid, axes through 0) — see [`shared/chart_render.py`](../../shared/chart_render.py).

...
- **`render_chart`** — deterministic charts: nested **`spec`** *or* **top-level** fields (`kind`, `series`, `title`, …). For **line**/**scatter** in «school» style: `style: "school"` and/or `axes_origin: true`, optional **`grid_step`** (default **1**) for major grid/tick step; use **`points`:** `{ x, y, label?, color? }` for marked intersections/vertices — **not** a two-point **`series`** (that draws a **segment**). For smooth curves, use **many** samples; keep separate branches in **separate** `series` (e.g. hyperbola arms) so lines do not connect across a gap. See [`shared/chart_render.py`](../../shared/chart_render.py).
- **`render_geometry`** — deterministic 2D/3D diagrams: **`spec`** *or* top-level fields (`mode`, `segments`, `vertices`, …); segments, circles, arcs, labeled points, or 3D vertices+edges ([`shared/geometry_render.py`](../../shared/geometry_render.py)); for school-style geometry when labels matter.

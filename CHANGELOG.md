...
### Fixed
- **Оркестратор + Telegram: «зависло» после долгого запроса** — параллельные `POST /api/v1/tasks` для одного `user_id` больше не гоняются в одном `ToolDispatcher`: неблокирующая блокировка на пользователя (heartbeat не затрагивается); второй запрос сразу получает `status: "failed"` с текстом про ожидание или `/stop`. При **httpx ReadTimeout** у бота (ожидание ответа оркестратора, по умолчанию 120 с) в оркестратор уходит **`POST /api/v1/tasks/cancel`**, чтобы снять флаг между раундами LLM/инструментов. Файлы: [`services/orchestrator/agent.py`](services/orchestrator/agent.py), [`shared/telegram_app/balbes_bot.py`](shared/telegram_app/balbes_bot.py); тест [`tests/unit/test_orchestrator_user_lock.py`](tests/unit/test_orchestrator_user_lock.py).

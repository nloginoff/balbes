...
### Changed
- Экспорт чатов Memory: Redis из env без полного `Settings`; при отсутствии прав на `/data_for_agent` — запись в `data_for_agent/` в корне репо ([.gitignore](.gitignore)); `EXPORT_CHATS_OUTPUT` / `--output`; [`docs/ru/DEPLOYMENT.md`](docs/ru/DEPLOYMENT.md).

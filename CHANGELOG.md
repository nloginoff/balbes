...
- **Dev-blog пакет постов: «планировщик дал темы, писатели пусто»** — у писателей увеличен `max_tokens` (4096), вход **обрезается** по длине (меньше переполнения контекста), JSON парсится с **вырезанием** объекта ` {...} ` из мусора; при полном провале пакетных вызовов — **один** черновик без разбиения по темам. [`services/blogger/agent.py`](services/blogger/agent.py)
- **`list_drafts` (инструмент блогера)** — в [`shared/agent_tools/registry.py`](shared/agent_tools/registry.py) после списка ID запрашивается **GET /api/v1/posts/{id}** для каждого поста: в ответе списка нет поля `content`, поэтому владелец в чате видел заголовки без текстов. Описание инструмента обновлено; system prompt бизнес-бота: явно вызывать `list_drafts` для полных RU/EN.
- **Одобрение черновиков блогера** — при `BUSINESS_BOT_TOKEN` превью с inline-кнопками идут в **бизнес-бот**; обработчик `blog_approve`/`edit`/`reject` в [`services/blogger/business_bot.py`](services/blogger/business_bot.py). У оркестратора [`config/agents/balbes.yaml`](config/agents/balbes.yaml) — `telegram.blog_callbacks: false`. Публикация в каналы по-прежнему через основной бот ([`TelegramPublisher.publish_to_channel`](services/blogger/publisher.py)).
...
### Changed
- **`render_chart` (школьный режим и точки)** — при `style: "school"` / `axes_origin` для line/scatter основные **деления сетки и тиков** задаются шагом **`grid_step`** (по умолчанию **1**), с ограничением плотности тиков (длинный диапазон → более крупный шаг). Отдельно поле **`points`**: `{ x, y, label?, color? }` — маркеры с подписями, не «двухточечный series»-отрезок. **Оркестратор + OpenRouter:** при ошибке `chat/completions` в лог пишется превью тела ответа. **HTTP 400** не трогает **fallback**-цепочку (запрос не «переезжает» на MiniMax вместо выбранной модели; см. [`services/orchestrator/agent.py`](services/orchestrator/agent.py)). См. [`shared/chart_render.py`](shared/chart_render.py).

### Fixed
- **`render_chart` / `render_geometry` + битая JSON-строка `spec`** — при синтаксической ошибке в `spec` (строка) выполняется **merge полей с верхнего уровня** (`kind`, `series`, …), если модель продублировала их. Иначе — понятное сообщение. [`shared/agent_tools/registry.py`](shared/agent_tools/registry.py), тест в [`tests/unit/test_chart_geometry_render.py`](tests/unit/test_chart_geometry_render.py).
